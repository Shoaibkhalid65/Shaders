...
import com.example.shaders.ui.theme.Pink80
import com.example.shaders.ui.theme.Purple80
import com.example.shaders.ui.theme.PurpleGrey80
...
fun CurtainTransition(
    ...
    Box(
        ...
        if (state.isAnimationRunning || state.progress(state.settledValue, state.targetValue) < 1.0 || state.currentValue == DragAnchors.End) {
            content()
        }
        if (state.isAnimationRunning || state.progress(state.settledValue, state.targetValue) < 1.0) {
            Box(
                modifier = Modifier
                    .graphicsLayer {
